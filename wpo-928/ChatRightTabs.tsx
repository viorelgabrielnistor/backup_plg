...
const ChatRightTabs: FC<IChatRightTabs> = ({
  ...
  const hasEmptyTranslatedText = translations.some(trs => trs.translatedText === '');
      ...
      >
        ...
        >
          <div className={ styles.tabsContent } ref={ historyRef } onScroll={ handleScroll }>
            {
              hasEmptyTranslatedText && <MessageDeleted title={ intl.formatMessage({ id: 'label.tabs.messageDeleted' }) } />
            }
            { hasScrolledNotification && scrollButton }
            {
              translations.map(({ type, text, translatedText, id, status, rejectionReason, date, copied, withConfidence }, index) => {
                ...
                if (type === TICKET_TYPE.ORIGINAL && translatedText) {
                  return (
                    <CustomerReply
                      key={ type + id }
                ...
                if (status !== STATUS_TYPE.REJECTED && translatedText) {
                  return (
                    <AgentReply
                      key={ type + id }
                      status={ status }
          ...
          <div className={ styles.tabsContent } ref={ historyCustomerRef } onScroll={ handleScroll }>
            ...
            }
            {
              translations.map(({ type, text, translatedText, id, status, rejectionReason, date, withConfidence }, index) => {
                ...
                if (type === TICKET_TYPE.REPLY && status !== STATUS_TYPE.REJECTED && translatedText) {
                  return (
                    <AgentReply
                      key={ type + id + index }
