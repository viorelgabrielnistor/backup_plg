...
const PendingTranslations: FC = () => {
  ...
  const handleReassignToClick = async (ticket: string | null, languageExpert: string): Promise<void> => {
    const tickets = ticket ? [ticket] : checkedTickets;
    setSelectedTickets(tickets);
    
    const agents = await getAvailableAgentsForTickets(tickets);
    const filteredAgents = agents.filter(agent => agent.label !== languageExpert)
              ...
              { pendingTranslations && pendingTranslations.map((row)=>(
                <tr key={ row.id } className={ `${checkedTickets[row.id] ? styles.rowSelected : ''}` }>
                  ...
                  <td className={ styles.colActions }>
                    ...
                    <DropdownButton
                      onReassign={ () => handleReassignToClick(row.id, role === USER_ROLES.TSSC_MANAGER ? row.languageExpert : row.agent ) }
