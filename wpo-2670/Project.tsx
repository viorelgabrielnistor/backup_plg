...
import UnsupSettings from './UnsupSettings';
import SupSettings from './SupSettings';
...
const Project: FC<{}> = () => {
  ...
  const handleWorkflowChange = (values: IProjectValues, setFieldTouched, setFieldValue) => {
    return (value, event, fieldValue): void => {
      ...
      const indexOfField = (input: string): number => fieldValue.indexOf(input);
      if (value === 'unsupervised') {
        if (event.target.checked) {
          fieldValue.splice(indexOfField('unsupervisedTimeframe'), 1)
          indexOfField('unsupervised') <0 && fieldValue.push('unsupervised');
        }
      }
      if (value === 'unsupervisedTimeframe') {
        if (event.target.checked) {
          indexOfField('unsupervised') >= 0 && fieldValue.splice(indexOfField('unsupervised'), 1);
          fieldValue.splice(indexOfField('unsupervisedTimeframe'), 1)
          fieldValue.push('unsupervisedTimeframe')
        } 
      }
      if (value === 'supervised') {
        if (event.target.checked) {
          fieldValue.splice(indexOfField('supervisedTimeframe'), 1)
          indexOfField('supervised') <0 && fieldValue.push('supervised');
        }
      }
      if (value === 'supervisedTimeframe') {
        if (event.target.checked) {
          indexOfField('supervised') >= 0 && fieldValue.splice(indexOfField('supervised'), 1);
          fieldValue.splice(indexOfField('supervisedTimeframe'), 1)
          fieldValue.push('supervisedTimeframe')
        }
      }
      console.log('final: ', fieldValue);
      setFieldValue('workflows',fieldValue)
    };
  };
  ...
  const copyProject: IProjectValues = {
    ...project,
    projectSla: {
      ...project.projectSla,
      unsupervisedTranslation: project.projectSla.workingSchedule,
      supervisedTranslation: project.projectSla.workingSchedule,
      unsupervisedAlways: true,
      supervisedAlways: true,
      unsupervisedTimeframe:[],
      supervisedTimeframe:['07/30/2022','08/08/2022','07/07/2022'],
      // unsupervisedTimeframe: ['Sun Jun 26 2022 00:00:00 GMT+0300 (Eastern European Summer Time)', 'Mon Jun 27 2022 10:00:00 GMT+0300 (Eastern European Summer Time)']
    },
    // workflows: [
    //   WORKFLOWS.UNSUPERVISED, WORKFLOWS.UNSUPERVISED_TIMEFRAME, WORKFLOWS.SUPERVISED, WORKFLOWS.SUPERVISED_TIMEFRAME
    // ]
    workflows: [
      WORKFLOWS.UNSUPERVISED, WORKFLOWS.SUPERVISED
    ]
  }
  console.log('copyProject: ', copyProject)

  return (
    <Formik
      initialValues={ copyProject }
      ...
      onSubmit={ (values, { setSubmitting, setFieldError }) => {
        console.log('final form of values: ',values)
        // try {
        //   if (projectId) {
        ...
        <>
          ...
          <Form onSubmit={ handleSubmit }>
              ...
              <Col>
                <Field
                  ...
                  checkboxValues={ [
                    ...
                    getLabelValue(intl.formatMessage({ id: 'label.project.unsupervisedTimeframe' }), WORKFLOWS.UNSUPERVISED_TIMEFRAME),
                    getLabelValue(intl.formatMessage({ id: 'label.project.supervised' }), WORKFLOWS.SUPERVISED),
                    getLabelValue(intl.formatMessage({ id: 'label.project.supervisedTimeframe' }), WORKFLOWS.SUPERVISED_TIMEFRAME),
                  ] }
            ...
            <Row>
              <Col xs={ 6 }>
                {
                  values.workflows.includes(WORKFLOWS.UNSUPERVISED_TIMEFRAME) &&
                  <UnsupSettings
              ...
              <Col xs={ 6 }>
                {
                  values.workflows.includes(WORKFLOWS.SUPERVISED_TIMEFRAME) && 
                <SupSettings
                  setFieldValue={ setFieldValue }
                  setFieldTouched={ setFieldTouched }
                  projectSla={ values.projectSla }
