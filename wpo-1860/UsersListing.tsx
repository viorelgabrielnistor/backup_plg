...
import PaginationItems from '../shared/pagination/Pagination';
...
const UsersListing: FC<{}> = () => {
  ...
  const total: number | any = usersForDisplay && usersForDisplay.length;
  ...
  useEffect(() => {
    total && setPageCount(Math.ceil(total/limit));
  },[usersForDisplay, limit, total ])
  ...
  const handleRedirect = (path): void => {
    history.push(path);
  };
  ...
  const currentPage = cpg;
  const start = limit * (currentPage - 1);
  const paginatedUsers = limit !==  users?.length  ? usersForDisplay?.slice(start, start + limit) : usersForDisplay;
      ...
      <Row>
        ...
        <Col className="text-right" md={ 6 }>
          <button
            ...
            type="button"
            onClick={ () => handleRedirect(routePaths.admin.userNew) }
          >
        ...
        <Col md={ 12 }>
          {
            ...
            paginatedUsers && paginatedUsers.length > 0 ?
              ( <>
                                  ...
                                  editable ? (
                                    <>
                                      <Button
                                        ...
                                        variant="link"
                                        onClick={ () => handleRedirect(toPath(routePaths.admin.userEdit, { userId: id! })) }
                                      >
                ...
                <div className={ styles.paginationConntainer }>
                  <PaginationItems pageCount={ pageCount } limit={ limit } setLimit={ setLimit } total={ total } setCpg={ setCpg }/>
                </div>
              </>
              ) : isLoadingUsers ? (
